...
(async () => {
  const htmlPath = 'file:///' + path.resolve(__dirname, 'guia-pruebas-soporte-coordinacion.html').replace(/\\/g, '/');
  const out = path.resolve(__dirname, 'Guia-Pruebas-Soporte-Coordinacion.pdf');
  ...
  await page.pdf({
    ...
    footerTemplate: `<div style="width:100%; font-size:7pt; color:#94a3b8; font-family:monospace; padding:0 14mm; display:flex; justify-content:space-between;">
        <span>Guía de pruebas — soporte y coordinación (sesión con Fanely)</span>
