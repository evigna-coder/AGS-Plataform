...
- @.claude/rules/release-flow.md — sistema-modular tiene auto-update vía GH Actions; bug fixes y features visibles al user deben ir por el flow de release (`pnpm release:patch/minor`) para llegar a las PCs instaladas
- @.claude/rules/establecimiento-autoselect.md — en todo flujo de creación, si el cliente tiene un único establecimiento debe autoseleccionarse; usar el helper `establecimientoUnicoId`
