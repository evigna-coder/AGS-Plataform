        ...
        >
          ...
          {isTableSection(section) && (() => {
            ...
            if (section.id === 'sec_19') {
              const titleRows = (rowsToPass ?? []).filter(
                (row) =>
                  (row.cells?.length ?? 0) === 2 &&
                  (row.cells?.[0] as { variant?: string })?.variant === 'header' &&
                  ((row.cells?.[0] as { colSpan?: number })?.colSpan ?? 1) >= 4
              );
              console.log('SEC19 TITLE ROWS', titleRows.map((row) => ({
                rowId: row.id,
                cell0Type: row.cells?.[0]?.type,
                cell0Text: row.cells?.[0]?.type === 'text' ? String((row.cells[0] as { value?: unknown }).value ?? '') : '',
                cell1Type: row.cells?.[1]?.type,
                cell1Label: row.cells?.[1]?.type === 'checkbox' ? String((row.cells[1] as { checkboxLabel?: string }).checkboxLabel ?? '') : (row.cells?.[1]?.type === 'text' ? String((row.cells[1] as { value?: unknown }).value ?? '') : ''),
              })));
            }
            return (
              <div
