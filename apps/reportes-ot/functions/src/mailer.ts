...
function buildCierreAdminEmail(data: any): { subject: string; html: string } {
 ...
 */
export const processMailQueue = functions.firestore.onDocumentCreated(
  'mailQueue/{mailId}',
  async (event) => {
    ...
    try {
      ...
      if (!smtpUser) {
        ...
        await snap.ref.update({ status: 'no_smtp', processedAt: FieldValue.serverTimestamp() });
        return;
      }
      ...
      if (mailData.type === 'cierre_admin_ot') {
        ...
        await snap.ref.update({ status: 'unknown_type', processedAt: FieldValue.serverTimestamp() });
        return;
      }
      ...
      await snap.ref.update({ status: 'sent', processedAt: FieldValue.serverTimestamp() });
