    ...
    if (activeId.startsWith('pending:')) {
      ...
      for (const ot of otsToAssign) {
        ...
        if (existing && continuaElRango(existing, targetFecha)) {
          // Hacia atrás extiende el inicio — no invierte el rango (2026-08-11).
          const patch = extenderRangoHacia(existing, targetFecha, targetQuarter);
          if (Object.keys(patch).length > 0) updateEntry(existing.id, patch);
        } else {
