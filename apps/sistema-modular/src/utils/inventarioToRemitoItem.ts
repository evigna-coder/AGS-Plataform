...
export function getRemitoItemCodigo(item: RemitoItem): string {
  ...
  // (LNR-xxxx) no se declara en ningún remito — si el artículo no tiene código,
  // la columna Producto dice "S/C" (2026-08-07).
  if (item.tipoEntidad === 'loaner') return item.articuloCodigo || 'S/C';
