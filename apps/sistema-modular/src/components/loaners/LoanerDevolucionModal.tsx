...
export function LoanerDevolucionModal({ open, onClose, clienteNombre, parteDescripcion = null, onConfirm }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose}
      title={esParte ? 'Registrar retorno de la parte' : 'Registrar devolucion'}
      subtitle={esParte ? `${parteDescripcion} · Cliente: ${clienteNombre}` : `Cliente: ${clienteNombre}`}
      footer={
      <div className="flex justify-end gap-2">
        <Button variant="secondary" size="sm" onClick={onClose}>Cancelar</Button>
        <Button variant="primary" size="sm" onClick={handleConfirm} disabled={!condicion.trim() || saving}>
          {saving ? 'Guardando...' : esParte ? 'Confirmar retorno' : 'Confirmar devolucion'}
        </Button>
      </div>
    }>
