  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title="" maxWidth="2xl">
          ...
          <div className="bg-slate-100 border border-slate-200 rounded-lg px-4 py-2.5 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="text-xs">
                ...
                {form.motivoAnulacion && <span className="text-slate-500"> — {form.motivoAnulacion}</span>}
              </div>
            </div>
          </div>
        )}
        ...
        {form.presupuestoOrigenId && (
          <div className="text-xs text-slate-500 mb-3 flex items-center gap-1">
            <span>Revisión de</span>
            <button onClick={() => loadRevisionHistory()} className="text-teal-600 hover:underline font-medium">
              {extractBase(form.numero)}
            </button>
            <span className="text-slate-300 mx-1">·</span>
            <button onClick={() => loadRevisionHistory()} className="text-slate-400 hover:text-slate-600 text-[10px]">
      ...
      <CreateRevisionModal
        open={showRevision}
        presupuesto={showRevision ? { id: presupuestoId, numero: form.numero, estado: form.estado, origenTipo: form.origenTipo as any, origenId: form.origenId } as Presupuesto : null}
