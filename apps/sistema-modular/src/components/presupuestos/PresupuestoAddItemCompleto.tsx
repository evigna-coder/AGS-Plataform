 ...
 * impuestos y disponibilidad). Convive con la carga rápida (wizard) — pedido
 * 2026-07-29: volver a ofrecer el enfoque anterior además del wizard.
 */
export function PresupuestoAddItemCompleto({ conceptosServicio, categoriasPresupuesto, moneda, onAdd, onClose }: Props) {
  const [articulos, setArticulos] = useState<Articulo[]>([]);
  const [item, setItem] = useState<Partial<PresupuestoItem>>({ cantidad: 1, unidad: 'unidad' });
