 ...
 */
export function RemitoItemsEditor({ items, unidades, maxCantidad, onAdd, onUpdate, onRemove }: Props) {
  // No ofrecer unidades ya agregadas.
  const opciones = useMemo(() => {
    ...
    return unidades
      ...
      .map(u => ({
        value: u.id,
        label: `${u.articuloCodigo} — ${u.articuloDescripcion}${u.nroSerie ? ` · S/N ${u.nroSerie}` : ''}${u.nroLote ? ` · Lote ${u.nroLote}` : ''} (${u.cantidad ?? 1} disp.)`,
  ...
  return (
    <div className="space-y-2">
      <div>
        <label className="block text-[11px] font-medium text-slate-500 mb-1">Agregar artículo del stock</label>
        <SearchableSelect
          value=""
