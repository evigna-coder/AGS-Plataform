...
export function HistorialDrawer({ qf, onClose }: Props) {
  useEffect(() => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
        <div className="shrink-0 flex items-start justify-between px-4 py-3 border-b border-slate-100 bg-teal-700 text-white">
          <div>
            <p className="text-[10px] font-mono uppercase tracking-wide text-teal-100">Historial</p>
            <p className="text-sm font-semibold">{qf.numeroCompleto}</p>
            <p className="text-xs text-teal-100 truncate">{qf.nombre}</p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg text-teal-100 hover:text-white hover:bg-teal-600"
          >
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono font-semibold text-slate-900">v{h.version}</span>
                      {esActual && (
                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-teal-700 text-white">Vigente</span>
                      )}
                    </div>
