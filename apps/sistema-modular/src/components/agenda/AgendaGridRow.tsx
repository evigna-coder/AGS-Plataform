...
interface AgendaGridRowProps {
  ...
  /** Días AGS globales (claves `${ingenieroId}_${fecha}`) — no laborable individual. */
  diasAgs?: Set<string>;
  rowHeight: string;
      ...
      {columns.map((col, idx) => {
        ...
        return (
          <AgendaGridCell
            ...
            entryVentaConcretada={entry?.ventaConcretada}
            entryPerIncident={entry?.perIncident}
            entryEsperaImportacion={entry?.esperaImportacion}
            ...
            isDiaAgs={diasAgs?.has(`${ingeniero.id}_${col.dateKey}`)}
