...
import { useUploadQueue, usePendingForUnidad } from '../../hooks/useUploadQueue';
import type { FotoUnidad, MomentoFotoUnidad } from '@ags/shared';

interface Props {
  unidadId: string;
  /** Codigo del articulo + serie/lote — se muestra en la cola global. */
  unidadEtiqueta: string;
  momento: MomentoFotoUnidad;
  /** Fotos ya confirmadas en Firestore, del mismo momento. */
  fotosConfirmadas?: FotoUnidad[];
  titulo: string;
}

/**
 * Captura multiple con camara nativa (mismo patron que CapturaFotosLoaner): las
 * fotos se encolan en IndexedDB y la cola offline las sube cuando hay senal. El
 * deposito no tiene buena senal, asi que se puede salir de la pantalla con
 * fotos pendientes sin perder nada.
 */
export function CapturaFotosUnidad({
  unidadId,
  unidadEtiqueta,
  momento,
  fotosConfirmadas = [],
  ...
  const { enqueueUnidad, retry, discard } = useUploadQueue();
  const pending = usePendingForUnidad(unidadId).filter(p => p.momento === momento);
