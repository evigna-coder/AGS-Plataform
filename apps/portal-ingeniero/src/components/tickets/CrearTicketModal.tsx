...
import { leadsService, clientesService, usuariosService } from '../../services/firebaseService';
import { MOTIVO_LLAMADO_LABELS, LEAD_AREA_LABELS, LEAD_ESTADO_LABELS, LEAD_ESTADO_ORDER } from '@ags/shared';
...
export default function CrearLeadModal({ open, onClose, onCreated }: Props) {
  ...
  const [nuevoEstado, setNuevoEstado] = useState<LeadEstado>('relevamiento_pendiente');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await leadsService.create({
        ...
        motivoLlamado,
        ...(motivoLlamado === 'otros' && motivoOtros.trim() ? { motivoOtros: motivoOtros.trim() } : {}),
        motivoContacto: '',
    ...
    <>
      <div>
        ...
        <select value={areaActual} onChange={e => setAreaActual(e.target.value as LeadArea | '')} className={selectClass}>
          ...
          {Object.entries(LEAD_AREA_LABELS).map(([value, label]) => (
