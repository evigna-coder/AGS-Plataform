...
export default function LeadAdjuntosSection({ leadId, adjuntos, onUpdated, readOnly }: Props) {
      ...
      <div className="p-4">
        ...
        {adjuntos.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {adjuntos.map(adj => (
              <div key={adj.id} className="group relative border border-slate-200 rounded-lg overflow-hidden bg-slate-50">
                {isImage(adj) ? (
                  <button type="button" onClick={() => setPreviewUrl(adj.url)} className="w-full aspect-square">
                ...
                {!readOnly && (
                  <button onClick={() => handleRemove(adj)}
