...
export function MLabel({ children, amber = false }: { children: ReactNode; amber?: boolean }) {
  return (
    <span className={`block font-mono text-[10px] font-semibold uppercase tracking-[0.16em] mb-2 md:mb-1.5 ${amber ? 'text-amber-700' : 'text-teal-700'}`}>
      {children}
    </span>
  );
}
...
export function GCard({ label, amber = false, children, className = '' }: {
  ...
  return (
    <div className={`rounded-2xl border px-4 py-3.5 md:py-2.5 ${amber ? 'bg-amber-100/70 border-amber-700/25' : 'bg-slate-50 border-slate-200'} ${className}`}>
...
export function FRow({ k, children, mono = false }: { k: string; children: ReactNode; mono?: boolean }) {
  return (
    <div className="grid grid-cols-[96px_1fr] gap-3.5 py-2 md:py-1.5 border-b border-slate-200 last:border-b-0 items-baseline text-[13.5px] md:text-[13px]">
