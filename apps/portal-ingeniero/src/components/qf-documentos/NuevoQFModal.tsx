...
export default function NuevoQFModal({ open, onClose, onCreated, defaultTipo = 'QF', defaultFamilia = 7 }: Props) {
  ...
  const preview = numero && !isNaN(familiaNum)
    ? `${formatQFNumeroCompleto(tipo, familiaNum, numeroPadded)}.01`
    : '—';

  const reset = () => {
    setTipo(defaultTipo); setFamilia(String(defaultFamilia));
    setNumero(''); setNombre(''); setDescripcion(''); setCambios('');
  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Descripción de la versión 01</label>
          <textarea
            value={cambios}
            onChange={(e) => setCambios(e.target.value)}
            rows={3}
            className="w-full border border-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
            placeholder="Qué contiene esta primera versión"
          />
          <p className="mt-1 text-[10px] text-slate-400">Se guarda como primera entrada del historial.</p>
        </div>
