...
**Post-research correction:** `SolicitudFacturacion` **ya existe** en `@ags/shared/types/index.ts:1280` + `facturacionService.ts` + página `/facturacion` con `FacturacionList.tsx`. Phase 10 **extiende** lo existente — NO crea colección nueva ni ruta hermana. Decisión user (2026-04-22): extender `/facturacion` con los campos faltantes + acciones admin (marcar enviada/facturada + nota contable). Dashboard gate por rol-de-fila: la lista sigue visible al contable; las acciones administrativas requieren `admin + admin_soporte`.

**Extensiones requeridas al shape existente:**
```ts
...
**Dashboard:** `/facturacion` existente (no hay ruta `/admin/solicitudes-facturacion` nueva) — extender con filtros adicionales vía `useUrlFilters`:
...
### Editor audit — post-research correction

El picker de artículos **funciona** para todos los tipos (user confirmó). El gap real identificado por research es:
- **StockAmplioIndicator wire-up** en `AddItemModal` al seleccionar artículo (Phase 9 building block sin consumer hoy)
- **Validación al aceptar** — computar ATP y gatear acceptance si no hay stock suficiente (hoy FLOW-03 crea requerimientos condicionales, pero el feedback al vendedor durante edición no existe)

Phase 10 NO rebuilds el picker — solo agrega el ATP indicator inline + validación.
...
- **Migración del export existente:** `exportVentasInsumosExcel.ts` hoy usa teal header fill — al implementar el helper genérico, el existente se migra a plain strip también (consistencia). Break visual esperado mínimo.
